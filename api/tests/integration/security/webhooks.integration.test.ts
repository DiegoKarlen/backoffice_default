...
import { WEBHOOK_SECRET_HEADER } from "../../../src/payments/middleware/verify-webhook.js";
import {
...
describe("[integration][security] payment webhooks", () => {
  ...
  const stubSecret = paymentsEnv.webhookStubSecret ?? "dev-webhook-stub-secret";
  ...
  it("POST stub webhook without secret returns 401", async (t) => {
    if (skipIfNoDatabase(t, db) || !http || !paymentsEnv.webhookStubEnabled) return;
  ...
  it("POST stub webhook with secret completes deposit and replay is idempotent", async (t) => {
    if (skipIfNoDatabase(t, db) || !http || !paymentsEnv.webhookStubEnabled) return;
    ...
    await prisma.deposit.update({
      where: { id: deposit.id },
      data: { externalRef: `stub-${deposit.id}` },
    });

    try {
      const headers = { [WEBHOOK_SECRET_HEADER]: stubSecret };
      const body = JSON.stringify({ depositId: deposit.id, success: true });

      const first = await apiFetch(http.baseUrl, "/webhooks/payments/stub", {
        method: "POST",
        headers,
        body,
      });
      ...
      assert.equal(wallet!.balanceCents, 5_000);

      const second = await apiFetch(http.baseUrl, "/webhooks/payments/stub", {
        method: "POST",
        headers,
        body,
      });
      ...
      const walletAfter = await prisma.wallet.findUnique({ where: { playerId: player.id } });
      assert.equal(walletAfter!.balanceCents, 5_000);
