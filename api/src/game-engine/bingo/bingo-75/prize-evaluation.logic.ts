...
/** Ganadores que completaron la figura en la primera bolilla de la partida (misma bolilla = todos). */

function winnersOnEarliestCompletionBall(eligible: EligibleCard[]): EligibleCard[] {
...
export function computeDeferredWinsAfterBall(params: {
  ...
  for (const prize of prizes) {
    ...
    for (const card of cardsWithCells75) {
      ...
      let completionIndex: number;

      if (prize.figure === "JACKPOT") {

        completionIndex = figureCompletionDrawIndex("FULL_HOUSE", card.cells75, drawnOrdered);

        if (completionIndex < 0) continue;

        const ballsWhenCompleted = completionIndex + 1;

        if (!params.jackpotMaxBall || ballsWhenCompleted >= params.jackpotMaxBall) continue;

      } else {

        completionIndex = figureCompletionDrawIndex(prize.figure, card.cells75, drawnOrdered);
    ...
    const toAward = winnersOnEarliestCompletionBall(eligible);
