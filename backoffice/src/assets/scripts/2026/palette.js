...
function buildItems() {
  ...
  items.push({
    ...
    section: t('palette.sectionAction'),
    action: () => {
  ...
  items.push({
    ...
    section: t('palette.sectionExternal'),
...
function refreshPaletteStaticUi() {
  if (!modal) return;
  modal.setAttribute("aria-label", t("palette.ariaLabel"));
  if (input) input.placeholder = t("palette.placeholder");
  const foot = modal.querySelector(".palette-foot");
  if (foot) {
    foot.innerHTML = `
