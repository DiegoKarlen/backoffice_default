 ...
 * @param {Array<{ figure: string; amount?: string | number }>} [prizes]
...
function prizesForEditor(prizes) {
  ...
  for (const p of prizes) {
    ...
    if (p?.figure && p.figure !== "JACKPOT") byFig.set(p.figure, p);
  ...
  return BINGO_PRIZE_FIGURES.map((figure) => {
    ...
    return { figure, enabled: false, amount: "1" };
 ...
 * @param {{ figure: string; enabled?: boolean; amount?: string }} item
...
function wirePrizeItemToggles(host) {
  ...
  host.querySelectorAll("[data-bo-prize-enabled]").forEach((cb) => {
    ...
    cb.addEventListener("change", () => {
      ...
      row.querySelectorAll("[data-bo-prize-amount]").forEach((el) => {
